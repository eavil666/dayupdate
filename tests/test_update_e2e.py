...
def test_update_flow_local_source(tmp_path):
    ...
    au = updater.AutoUpdater(current_version='1.6.1',
                             version_urls=[vj], exe_urls=[],
...
def test_update_no_new_version(tmp_path):
    ...
    au = updater.AutoUpdater(current_version='1.6.1', version_urls=[vj],
                             log_cb=lambda m: None)
...
def test_update_source_unreachable(tmp_path):
    """本地源缺失 → 检查失败并记录原因"""
    au = updater.AutoUpdater(current_version='1.6.1',
                             version_urls=[str(tmp_path / 'nope.json')],
                             log_cb=lambda m: None)
    assert au.check_update() is None

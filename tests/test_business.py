...
def test_format_online_result():
    assert (
        main.format_online_result(
            {"status": "success", "country": "中国", "regionName": "广东", "city": "广州", "isp": "电信"}
        )
        == "中国 广东 广州 电信"
    )
...
def test_is_excluded_ip(monkeypatch):
    ...
    monkeypatch.setattr(
        ipdb, "EXCLUDED_IP_NETWORKS", [ipaddress.ip_network("10.0.0.0/8"), ipaddress.ip_address("192.168.9.9")]
    )

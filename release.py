...
def set_pyproject_version(version):
    ...
    try:
        ...
        new_src, n = re.subn(r'(?m)^version\s*=\s*["\']([^"\']+)["\']', f'version = "{version}"', src, count=1)
        if n == 0:
            return False
...
def set_app_version(version):
    ...
    with open(path, encoding="utf-8") as f:
        content = f.read()
    new_content, n = APP_VERSION_RE.subn(rf"\g<1>{version}\g<3>", content, count=1)
    if n == 0:
...
def get_current_branch():
    ...
    try:
        r = subprocess.run(
            ["git", "branch", "--show-current"], cwd=script_dir, capture_output=True, text=True, timeout=15
        )
        b = (r.stdout or "").strip()
...
def git_commit_tag_push(version, token=None, branch="main"):
    ...
    if result.returncode == 0:
        ...
        subprocess.run(["git", "commit", "-m", f"release: v{version}"], cwd=script_dir, check=True, env=env)

    # 删除已存在的 tag（如果重复发布）
    subprocess.run(["git", "tag", "-d", f"v{version}"], cwd=script_dir, capture_output=True, env=env)
    subprocess.run(["git", "push", "origin", f":refs/tags/v{version}"], cwd=script_dir, capture_output=True, env=env)

    # 创建新 tag
    subprocess.run(
        ["git", "tag", "-a", f"v{version}", "-m", f"Release v{version}"], cwd=script_dir, check=True, env=env
    )
    log(f"已创建 tag: v{version}")

    # push 代码和 tag（目标分支由调用方指定，分支开发不触碰 main）
    log(f"推送到 GitHub（分支 {branch}）...")
    subprocess.run(["git", "push", "-u", "origin", branch], cwd=script_dir, check=True, env=env)
    subprocess.run(["git", "push", "origin", f"v{version}"], cwd=script_dir, check=True, env=env)
    log("代码和 tag 推送完成")
...
def main():
    ...
    parser.add_argument("--version", "-V", type=str, default=None, help="指定版本号（如 1.1.0），默认从 main.py 读取")
    parser.add_argument(
        "--branch",
        "-b",
        type=str,
    ...
    parser.add_argument("--skip-build", action="store_true", help="跳过打包步骤，直接上传已有 exe")

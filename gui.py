...
class DailyReportGUI:
    ...
    def _build_ui(self):
        ...
        Button(btn_frame, text="选择文件", command=self._select_files, width=15, font=("宋体", 10)).pack(side="left")
        Button(btn_frame, text="自动识别", command=self._auto_detect, width=15, font=("宋体", 10)).pack(
            side="left", padx=5
        )
        Button(btn_frame, text="清空列表", command=self._clear_files, width=15, font=("宋体", 10)).pack(side="right")
        ...
        self.file_listbox = Listbox(
            list_frame, yscrollcommand=scrollbar.set, font=("宋体", 9), selectmode="extended", height=5
        )
        ...
        Button(ip_import_frame, text="导入业务IP", command=self._load_biz_ips, width=12, font=("宋体", 10)).pack(
            side="left"
        )
        Label(ip_import_frame, textvariable=self.excluded_ip_label_var, font=("宋体", 9), fg="gray").pack(
            side="left", padx=(3, 15)
        )

        self.terminal_ip_label_var = StringVar(value="未导入")
        Button(ip_import_frame, text="导入终端IP表", command=self._load_terminal_ips, width=14, font=("宋体", 10)).pack(
            side="left"
        )
        Label(ip_import_frame, textvariable=self.terminal_ip_label_var, font=("宋体", 9), fg="gray").pack(
            side="left", padx=5
        )
        ...
        Entry(date_frame, textvariable=self.date_var, width=12, font=("宋体", 10)).pack(side="left", padx=5)
        ...
        Entry(geos_frame, textvariable=self.local_geos_var, width=28, font=("宋体", 10)).pack(side="left", padx=5)
        Label(geos_frame, text="(逗号分隔，自动与告警提取合并)", font=("宋体", 9), fg="gray").pack(side="left")
        ...
        self.work_summary_text = Text(
            work_inner, font=("宋体", 10), yscrollcommand=work_scroll.set, height=3, wrap="word"
        )
        ...
        self.follow_items_text = Text(
            follow_inner, font=("宋体", 10), yscrollcommand=follow_scroll.set, height=3, wrap="word"
        )
        ...
        self.intel_items_text = Text(
            intel_inner, font=("宋体", 10), yscrollcommand=intel_scroll.set, height=3, wrap="word"
        )
        ...
        self.log_text = Text(log_inner, font=("Consolas", 9), state=DISABLED, wrap=NONE, bg="#f5f5f5")
        ...
        self.run_btn = Button(
            btn_frame, text="开始生成", command=self._run, font=("宋体", 12, "bold"), width=20, bg="#4CAF50", fg="white"
        )
        self.run_btn.pack(side="left", padx=5)

        self.open_ip_btn = Button(
            btn_frame, text="打开IP分析结果", command=self._open_ip_report, font=("宋体", 10), width=20, state=DISABLED
        )
        self.open_ip_btn.pack(side="left", padx=5)

        self.open_report_btn = Button(
            btn_frame, text="打开值守日报", command=self._open_daily_report, font=("宋体", 10), width=20, state=DISABLED
        )
        ...
        try:
            ...
            def worker():
                try:
                    ...
                    self.daily_report_path = generate_daily_report(
                        path_objects, date, work_summary, follow_items, intel_items, local_geos=local_geos
                    )

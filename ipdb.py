...
def extract_zones_from_alerts(files):
    ...
    if zones:
        _log(
            f"[+] 从告警文件提取内网区域 {len(zones)} 个: {', '.join(sorted(zones)[:8])}{'...' if len(zones) > 8 else ''}"
        )
    return zones
...
def load_probes_from_excel():
    ...
    try:
        ...
        ip_col = next(
            (c for c in ("IP地址", "IP", "ip") if c in df.columns), df.columns[1] if len(df.columns) > 1 else name_col
        )
        rows = []
...
def load_config(files=None, local_geos=None):
    ...
    nets = [
        ipaddress.ip_network(x.strip()) for x in ranges_raw.splitlines() if x.strip() and not x.strip().startswith("#")
    ]
    terminal_ips = set()
    ...
    conf["probes"] = [
        tuple(x.strip().split("|"))
        for x in probes_raw.splitlines()
